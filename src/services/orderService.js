...
export function quote(items, promo) {
  ...
  let discount = 0;
  let shipping = subtotal >= shippingConfig.freeShippingThreshold || subtotal === 0 ? 0 : shippingConfig.flatRate;
  ...
  return { subtotal, discount, shipping, total: Math.max(subtotal - discount + shipping, 0), promoApplied: !!rule };
}
